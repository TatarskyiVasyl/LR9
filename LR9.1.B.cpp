...
string thirdSubject(Specialty sp)
{
    switch (sp)
    {
    case COMP_SCIENCE: return "Програмування";
    case INFORMATICS:  return "Числ. методи";
    default:           return "Педагогіка";
    }
}
...
void Print(Student* s, const int N)
{
    ...
    cout << "| №  | Прізвище             | Курс | Спеціальність              "
        "| Фіз | Мат | Третя дисципліна     | Оц. |" << endl;
    cout << "------------------------------------------------------------------"
        "------------------------------------------" << endl;
...
for (int i = 0; i < N; i++)
{
    ...
    switch (s[i].specialty)
    {
    case COMP_SCIENCE: third = s[i].programming; break;
    case INFORMATICS:  third = s[i].num_methods;  break;
    default:           third = s[i].pedagogy;     break;
    }
    ...
    cout << "| " << setw(20) << left << thirdSubject(s[i].specialty) << " ";
    cout << "| " << setw(3) << right << third << " |" << endl;
}
...
double Task1(Student* s, const int N)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (s[i].phys >= 4 && s[i].math >= 4 && third >= 4)
            k++;
    }
    return 100.0 * k / N;
}
