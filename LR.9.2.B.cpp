...
string thirdSubject(Specialty s)
{
    if (s == COMPUTER_SCIENCE) return "Програмування";
    if (s == INFORMATICS)      return "Чисельні методи";
    return "Педагогіка";
}

int getThird(const Student& st)
{
    if (st.specialty == COMPUTER_SCIENCE) return st.programming;
    if (st.specialty == INFORMATICS)      return st.numerical_methods;
    return st.pedagogy;
}

void setThird(Student& st, int val)
{
    if (st.specialty == COMPUTER_SCIENCE) st.programming = val;
    else if (st.specialty == INFORMATICS) st.numerical_methods = val;
    else                                  st.pedagogy = val;
}
...
void Create(Student* p, const int N)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        cout << "  оцінка з " << thirdSubject(p[i].specialty) << ": ";
        cin >> g3;
        setThird(p[i], g3);
...
{
    cout << "=========================================================================================" << endl;
    cout << "| №  |   Прізвище    | Курс |      Спеціальність       | Фіз. | Мат. | Прогр| Чис.м| Пед.|" << endl;
    cout << "-----------------------------------------------------------------------------------------" << endl;
    for (int i = 0; i < N; i++)
    {
        ...
        case COMPUTER_SCIENCE:
            cout << "| " << setw(5) << right << p[i].programming << "|      |     |";
            break;
        case INFORMATICS:
            cout << "|      | " << setw(5) << right << p[i].numerical_methods << "|     |";
            break;
        default:
            cout << "|      |      | " << setw(3) << right << p[i].pedagogy << " |";
            break;
        }
        cout << endl;
    }
